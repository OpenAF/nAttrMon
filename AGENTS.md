...
When adding, or changing the `execArgs` of, any `config/objects/nInput_*.js|nOutput_*.js|nValidation_*.js` constructor, also add or update its `config/objects.meta/<Constructor>.yaml` entry -- see `docs/OBJECTS-METADATA.md` for the required fields (`kind`, `title`, `description`, `examples`, `arguments` with `name`/`type`/`description`/`example`) and its contributor checklist. This is documentation, not enforced repo-wide (many existing plugs predate it and have no metadata yet), but any new metadata file you do add is validated by `test_phase6_objects_meta.js` as part of the hermetic suite.
