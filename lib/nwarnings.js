...
nWarnings.prototype.setHealExecuted = function(aTitle) {
	print("SET HEAL " + aTitle);
	var w = $ch(this.chWarnings).get({ "title": aTitle });
	if (isUnDef(w.healingArgs)) w.healingArgs = {};
	w.healingArgs.__executed = true;
	this.setWarningByName(aTitle, w);
	print("SET HEAL DONE " + aTitle);
};

nWarnings.prototype.getHealArgs = function(aTitle) {
	var w = $ch(this.chWarnings).get({ "title": aTitle });
	return (isDef(w) ? w.healingArgs : w);
};

nWarnings.prototype.getHealExecuted = function(aTitle) {
	var w = $ch(this.chWarnings).get({ "title": aTitle });
	if (isUnDef(w)) return false;
	if (isUnDef(w.healingArgs)) w.healingArgs = {};
	return w.healingArgs.__executed;
};