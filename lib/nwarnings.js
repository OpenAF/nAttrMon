// nAttrMon warnings functionality
// Copyright 2023 Nuno Aguiar

// Warnings collection wrapper
// ----------------------------------------
// Manages warning and notification channels
// ----------------------------------------
 var nWarnings = function () {
	...
	if (isDef(__NAM_CHANNEL_WARNS)) {
		...
		if (!isMap(o)) logWarn("Couldn't parse __NAM_CHANNEL_WARNS to a map")
	...
	if (isDef(__NAM_CHANNEL_WNOTS)) {
		var o = jsonParse(__NAM_CHANNEL_WNOTS, true)
		if (!isMap(o)) logWarn("Couldn't parse __NAM_CHANNEL_WNOTS to a map")
		o.options = __nam_getSec(o.options)
		o = __nam_getChExtraOptions(o, "title")
		$ch(this.chNotifications).create(1, o.type, o.options)
	} else {
		this.chNotifications = this.chWarnings
	}
	...
	if (__NAM_CLOSEDHK_HOWLONGAGOINMS > 0) {
		...
		if (__NAM_CLOSEDHK_ONSTARTUP) {
			$from($cache("nattrmon::warnings::cache").get({}))
			.equals("level", nWarning.LEVEL_CLOSED)
			.select((r) => {
				if ((now() - (new Date(r.lastupdate)).getTime()) > __NAM_CLOSEDHK_HOWLONGAGOINMS) {
					parent.getCh().unset({ title: r.title })
					if (isDef(__NAM_CHANNEL_WNOTS)) $ch(parent.chNotifications).unset({ title: r.title })
				}
			})
		}
		// Set the a trigger on set/setall
		// Throttled to at most once per __NAM_CLOSEDHK_HOWLONGAGOINMS window: a closed
		// warning only ever becomes eligible for removal after that window elapses, so
		// scanning more often than that buys nothing.
		this.__closedHkLastScan = $atomic(0, "long")
		this.getCh().subscribe((aC, aO, aK, aV) => {
			if (aO == "set" || aO == "setall") {
				var _now = now()
				if ((_now - parent.__closedHkLastScan.get()) < __NAM_CLOSEDHK_HOWLONGAGOINMS) return
				parent.__closedHkLastScan.set(_now)
...
// Get underlying warnings channel
// ----------------------------------------
// Returns channel handler
// ----------------------------------------
nWarnings.prototype.getCh = function () {
	return $ch(this.chWarnings)
}

// Upsert warning entry
// ----------------------------------------
// aWarning = warning map or instance
// ----------------------------------------
nWarnings.prototype.setWarning = function(aWarning) {
	// Validations
	if (isUnDef(aWarning) || Object.keys(aWarning).length == 0) return
	...
	if (update) {
		...
		//if (isDef(__NAM_CHANNEL_WNOTS)) $ch(parent.chNotifications).set({ title: toAdd.title }, toAdd.getData())
	}
}

// Get all warnings grouped by level
// ----------------------------------------
// Returns map of arrays per level
// ----------------------------------------
...
// Replace warnings from snapshot
// ----------------------------------------
// aWarningsSnapshot = array/map of warnings
// ----------------------------------------
nWarnings.prototype.setWarnings = function (aWarningsSnapshot) {
	this.addWarnings(aWarningsSnapshot)
}

// Get warning by title
// ----------------------------------------
// anWarningName = warning title
// Returns warning instance or undefined
// ----------------------------------------
...
// ----------------------------------------
// anWarningName = notification title
// Returns notification entry
// ----------------------------------------
nWarnings.prototype.getNotificationByName = function(anWarningName) {
	if (isDef(__NAM_CHANNEL_WNOTS)) {
		return $ch(this.chNotifications).get({ "title": anWarningName })
	} else {
		return this.getWarningByName(anWarningName)
	}
}

// Update warning entry by title
// ----------------------------------------
// anWarningName = warning title
// aWarning      = warning data
// ----------------------------------------
nWarnings.prototype.setWarningByName = function (anWarningName, aWarning) {
	...
	//if (isDef(__NAM_CHANNEL_WNOTS)) $ch(this.chNotifications).getSet({ title: anWarningName }, { title: anWarningName }, aWarning)
}

// Update notification entry by title
// ----------------------------------------
// anWarningName = notification title
// aWarning      = warning data
// ----------------------------------------
nWarnings.prototype.setNotificationByName = function(anWarningName, aWarning) {
	if (isDef(__NAM_CHANNEL_WNOTS)) {
		$ch(this.chNotifications).set({ title: anWarningName }, aWarning)
	} else {
		$ch(this.chWarnings).getSet({ title: anWarningName }, { title: anWarningName }, aWarning)
	}
}
...
// ----------------------------------------
// arrayOfWarnings = array of warning structs
// ----------------------------------------
