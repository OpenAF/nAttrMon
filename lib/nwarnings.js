...
var nWarnings = function () {
	...
	if (BUFFERCHANNELS) {
		...
		$ch(this.chWarnings + "::__buffer").create(1, "buffer", {
		...
		$ch(this.chWarnings).subscribe(function(aC, aO, aK, aV) {
			...
			switch(aO) {
			case "set"   : $ch(parent.chWarnings + "::__buffer").set(aK, aV);    break;
			case "setall": $ch(parent.chWarnings + "::__buffer").setAll(aK, aV); break;
			case "unset" : $ch(parent.chWarnings + "::__buffer").unset(aK);      break;
			}
		});
	}
