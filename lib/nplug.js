...
var nPlug = function(aInputMeta, args, aObject) {
	...
	// Use OpenAF atomics for thread-safe stats updates
	this.lastExecTimeInMs   = $atomic(-1, "long")
	this.numberOfExecs      = $atomic(0, "long")
	this.numberOfExecsInError = $atomic(0, "long")
	this.numberOfRunning    = $atomic(0, "long")
	this.avgExecTimeInMs    = $atomic(0, "long")
...
nPlug.prototype.exec = function(aScope, extraArgs) {
	...
	this.numberOfRunning.inc()
	this.touch(true)
	var init = now(), res, excep, inError = false
	try {
		if (isDef(this.aExecPre)) {
			var _dbgPre = nattrmon.debugFlag && isDef(ow.loadDebug)
			if (isUnDef(this.__fnExecPre) || this.__fnExecPreDbg !== _dbgPre) {
				var _srcPre = _dbgPre ? ow.debug.debug(this.aExecPre, __, true) : this.aExecPre
				this.__fnExecPre = new Function("scope", "args", _srcPre)
				this.__fnExecPreDbg = _dbgPre
			}
			var ret = this.__fnExecPre(aScope, argsToSend)
			if (isDef(ret)) argsToSend = ret
		}
		res = this.aObject.exec(aScope, argsToSend, this)
		if (isDef(this.aExecPos)) {
			if (isUnDef(this.__fnExecPos)) {
				this.__fnExecPos = new Function("value", "scope", "args", this.aExecPos)
			}
			var ret = this.__fnExecPos(res, aScope, argsToSend)
			if (isDef(ret)) res = ret
		}
		this.lastExecTimeInMs.set(now() - init)
		this.numberOfExecs.inc()
		// Compute moving average in milliseconds (rounded) using atomics
		var _n  = this.numberOfExecs.get()
		var _le = this.lastExecTimeInMs.get()
		var _pa = this.avgExecTimeInMs.get()
		var _na = (_n <= 1) ? _le : Math.round((((_n - 1) * _pa) + _le) / _n)
		this.avgExecTimeInMs.set(_na)
		this.numberOfRunning.dec()								
	} catch(e) {
		inError = true
		excep = e
		this.numberOfExecsInError.inc()
		this.numberOfRunning.dec()
		this.touch(true)
	}
...
nPlug.prototype.close = function() {
	try {
		if (isDef(this.aObject) && isFunction(this.aObject.close)) this.aObject.close()
	} catch(e) {
		logErr(`${this.aName} | Error closing plug: ${__nam_err(e, true, true)}`)
		return e
	}
}
...
nPlug.prototype.touch = function(dontTouchLast) {
	...
	$ch(this.chPlugs).set({ 
		...
		stats: {
			lastExecTimeInMs: (isDef(this.lastExecTimeInMs) && isDef(this.lastExecTimeInMs.get)) ? this.lastExecTimeInMs.get() : this.lastExecTimeInMs,
			avgExecTimeInMs: (isDef(this.avgExecTimeInMs) && isDef(this.avgExecTimeInMs.get)) ? this.avgExecTimeInMs.get() : this.avgExecTimeInMs,
			numberOfExecs: (isDef(this.numberOfExecs) && isDef(this.numberOfExecs.get)) ? this.numberOfExecs.get() : this.numberOfExecs,
			numberOfExecsInError: (isDef(this.numberOfExecsInError) && isDef(this.numberOfExecsInError.get)) ? this.numberOfExecsInError.get() : this.numberOfExecsInError,
			numberOfRunning: (isDef(this.numberOfRunning) && isDef(this.numberOfRunning.get)) ? this.numberOfRunning.get() : this.numberOfRunning
		}
	})
}
...
// ----------------------------------------
nPlug.prototype.getDescription = function() { return this.aDescription; }
...
// ----------------------------------------
nPlug.prototype.getStamp = function() { return this.aStamp; }
