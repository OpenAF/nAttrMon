	...
	if((aOp == "set" || aOp == "setall")) {
		...
		for(var i in aVs) {
			...
			if (isUnDef(v.lastupdate)) {
				v.lastupdate = new Date(); modified = true
			} else if (isString(v.lastupdate) && !isNaN(new Date(v.lastupdate).getTime())) {
				v.lastupdate = new Date(v.lastupdate); modified = true
			}

			if (isUnDef(v.createdate)) {
				v.createdate = new Date(); modified = true
			} else if (isString(v.createdate) && !isNaN(new Date(v.createdate).getTime())) {
				v.createdate = new Date(v.createdate); modified = true
			}

			if (modified) changes.push(v)
		}
		if (changes.length > 0) {
