...
if (io.fileExists(NATTRMON_SUBHOME + "/nattrmon.yaml"))
	pms = merge(io.readFileYAML(NATTRMON_SUBHOME + "/nattrmon.yaml"), pms);
...
if (isDef(pms.CLOSEDHK_HOWLONGAGOINMS)) __NAM_CLOSEDHK_HOWLONGAGOINMS = pms.CLOSEDHK_HOWLONGAGOINMS;
...
var __nam_getSec = aMap => {
	aMap = _$(aMap).isMap().default({});
	if (isDef(aMap.secKey)) {
		return merge(aMap, $sec(aMap.secRepo, aMap.secBucket, aMap.secPass, aMap.secMainPass, aMap.secFile).get(aMap.secKey));
	} else {
		return aMap;
	}
}

var __nam_getChExtraOptions = aMap => {
	if (aMap.type == "elasticsearch") { aMap.options.idKey = "id"; aMap.options.fnId = k => { return sha256(stringify(k, __, true)) } } 
	return aMap;
}
...
const nAttrMon = function(aConfigPath, debugFlag) {
	...
	if (isDef(__NAM_CHANNEL_LVALS)) {
		...
		o.options = __nam_getSec(o.options);
		o = __nam_getChExtraOptions(o);
		$ch(this.chLastValues).create(1, o.type, __nam_getSec(o.options));
	} else {
