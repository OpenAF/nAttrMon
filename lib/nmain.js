...
var __NAM_COREOBJECTS_LAZYLOADING   = false;
...
if (io.fileExists(NATTRMON_SUBHOME + "/nattrmon.yaml")) {
	var pms = getEnvs();
	pms = merge(pms, io.readFileYAML(NATTRMON_SUBHOME + "/nattrmon.yaml"));

	//if (isUnDef(pms) || pms == null) pms = {};
	if (isDef(pms.JAVA_ARGS) && isArray(pms.JAVA_ARGS)) __NAM_JAVA_ARGS = pms.JAVA_ARGS;
	if (isDef(pms.LOGAUDIT)) __NAM_LOGAUDIT = pms.LOGAUDIT;
	if (isDef(pms.LOGAUDIT_TEMPLATE) && isString(pms.LOGAUDIT_TEMPLATE)) __NAM_LOGAUDIT_TEMPLATE = pms.LOGAUDIT_TEMPLATE;
	if (isDef(pms.LOGHK_HOWLONGAGOINMINUTES) && isNumber(pms.LOGHK_HOWLONGAGOINMINUTES)) __NAM_LOGHK_HOWLONGAGOINMINUTES = pms.LOGHK_HOWLONGAGOINMINUTES; 
	if (isDef(pms.NUMBER___NAM_WORKERS)) { __cpucores = Number(pms.NUMBER___NAM_WORKERS); __NAM_WORKERS = Number(pms.NUMBER___NAM_WORKERS); }
	if (isDef(pms.LOG_ASYNC)) __logFormat.async = pms.LOG_ASYNC;
	if (isDef(pms.DEBUG)) __NAM_DEBUG = pms.DEBUG;	
	if (isDef(pms.LOGCONSOLE)) __NAM_LOGCONSOLE = pms.LOGCONSOLE;
	if (isDef(pms.MAXPLUGEXECUTE_TIME)) __NAM_MAXPLUGEXECUTE_TIME = pms.MAXPLUGEXECUTE_TIME;
	if (isUnDef(params.withDirectory) && isDef(pms.CONFIG)) params.withDirectory = pms.CONFIG;

	if (isDef(pms.BUFFERCHANNELS)) __NAM_BUFFERCHANNELS = pms.BUFFERCHANNELS;
	if (isDef(pms.BUFFERBYNUMBER)) __NAM_BUFFERBYNUMBER = pms.BUFFERBYNUMBER;
	if (isDef(pms.BUFFERBYTIME))   __NAM_BUFFERBYTIME = pms.BUFFERBYTIME;

	if (isDef(pms.COREOBJECTS))    __NAM_COREOBJECTS = pms.COREOBJECTS;
	if (isDef(pms.COREOBJECTS_LAZYLOADING)) __NAM_COREOBJECTS_LAZYLOADING = pms.COREOBJECTS_LAZYLOADING;
...
const nAttrMon = function(aConfigPath, debugFlag) {
	...
	if (__NAM_BUFFERCHANNELS) {
		$ch(this.chCurrentValues).subscribe(ow.ch.utils.getBufferSubscriber(this.chCurrentValues, [ "name" ], __NAM_BUFFERBYNUMBER, __NAM_BUFFERBYTIME));
	}
...
nAttrMon.prototype.loadPlugs = function() {
	...
	if (!__NAM_COREOBJECTS_LAZYLOADING) {
		if (isDef(__NAM_COREOBJECTS)) 
			this.loadPlugDir(__NAM_COREOBJECTS, "objects", ignoreList);
		else
...
nAttrMon.prototype.loadObject = function(yy, type) {
	...
	if (isDef(yy.execFrom)) {
		if (__NAM_COREOBJECTS_LAZYLOADING && type != "objects") {
			var aPath;
			if (isDef(__NAM_COREOBJECTS)) {
				aPath = __NAM_COREOBJECTS + "/" + yy.execFrom + ".js";
			} else {
