...
if (isDef(pms.SLOWDOWN))       __NAM_SLOWDOWN       = toBoolean(pms.SLOWDOWN)
if (isDef(pms.SLOWDOWN_WARNS)) __NAM_SLOWDOWN_WARNS = toBoolean(pms.SLOWDOWN_WARNS)
if (isDef(pms.SLOWDOWN_TIME))  __NAM_SLOWDOWN_TIME  = Number(pms.SLOWDOWN_TIME)
if (isDef(pms.NOPLUGFILES))    __NAM_NOPLUGFILES    = toBoolean(pms.NOPLUGFILES)
if (isDef(pms.PLUGSORDER))     __NAM_PLUGSORDER     = String(pms.PLUGSORDER)
if (isDef(pms.ALLOW_EVAL_EXECFROM)) __NAM_ALLOW_EVAL_EXECFROM = toBoolean(pms.ALLOW_EVAL_EXECFROM)
if (isDef(pms.MAIN_WATCHDOG_SLEEP)) __NAM_MAIN_WATCHDOG_SLEEP = Number(pms.MAIN_WATCHDOG_SLEEP)
if (isDef(pms.MAIN_WATCHDOG_STUCKFACTOR)) __NAM_MAIN_WATCHDOG_STUCKFACTOR = Number(pms.MAIN_WATCHDOG_STUCKFACTOR)
if (isDef(pms.MAIN_WATCHDOG_WARN_COOLDOWN)) __NAM_MAIN_WATCHDOG_WARN_COOLDOWN = Number(pms.MAIN_WATCHDOG_WARN_COOLDOWN)
if (isDef(pms.RUNTIME_METRICS)) __NAM_RUNTIME_METRICS = toBoolean(pms.RUNTIME_METRICS)
if (isDef(pms.RUNTIME_METRICS_PERIOD)) __NAM_RUNTIME_METRICS_PERIOD = Number(pms.RUNTIME_METRICS_PERIOD)
if (isDef(pms.RUNTIME_METRICS_CH)) __NAM_RUNTIME_METRICS_CH = String(pms.RUNTIME_METRICS_CH)
if (isDef(pms.RUNTIME_EVENTS_CH)) __NAM_RUNTIME_EVENTS_CH = String(pms.RUNTIME_EVENTS_CH)
if (isDef(pms.RUNTIME_OWMETRICS)) __NAM_RUNTIME_OWMETRICS = toBoolean(pms.RUNTIME_OWMETRICS)
if (isDef(pms.RUNTIME_OWMETRICS_NAME)) __NAM_RUNTIME_OWMETRICS_NAME = String(pms.RUNTIME_OWMETRICS_NAME)
if (isDef(pms.DEGRADED_ERROR_RATE_THRESHOLD)) __NAM_DEGRADED_ERROR_RATE_THRESHOLD = Number(pms.DEGRADED_ERROR_RATE_THRESHOLD)
if (isDef(pms.DEGRADED_MIN_EXECS)) __NAM_DEGRADED_MIN_EXECS = Number(pms.DEGRADED_MIN_EXECS)
if (isDef(pms.DEGRADED_TIMEOUT_HITS)) __NAM_DEGRADED_TIMEOUT_HITS = Number(pms.DEGRADED_TIMEOUT_HITS)
if (isDef(pms.DEGRADED_WATCHDOG_HITS)) __NAM_DEGRADED_WATCHDOG_HITS = Number(pms.DEGRADED_WATCHDOG_HITS)
...
if (isDef(pms.INTEGRITY_WARN)) __NAM_INTEGRITY_WARN = toBoolean(pms.INTEGRITY_WARN)
if (isDef(pms.INTEGRITY_STRICT)) __NAM_INTEGRITY_STRICT = toBoolean(pms.INTEGRITY_STRICT)
if (isDef(pms.DESCRIPTOR_SCHEMA_WARN)) __NAM_DESCRIPTOR_SCHEMA_WARN = toBoolean(pms.DESCRIPTOR_SCHEMA_WARN)
if (isDef(pms.DESCRIPTOR_SCHEMA_STRICT)) __NAM_DESCRIPTOR_SCHEMA_STRICT = toBoolean(pms.DESCRIPTOR_SCHEMA_STRICT)
if (isDef(pms.DESCRIPTOR_LEGACY_WARN)) __NAM_DESCRIPTOR_LEGACY_WARN = toBoolean(pms.DESCRIPTOR_LEGACY_WARN)
...
if (isDef(pms.ERRSTACK)) __NAM_ERRSTACK = toBoolean(pms.ERRSTACK)
...
nAttrMon.prototype.start = function() {
	...
	if (this.runtimeMetricsEnabled && isNumber(this.runtimeMetricsPeriod) && this.runtimeMetricsPeriod > 0) {
		...
		if (isUnDef(this.__runtimeMetricsThreadUuid)) {
			this.__runtimeMetricsThreadUuid = this.thread.addScheduleThreadWithFixedDelay((uuid) => {
			...
			if (entry.aTime >= 0 || isDef(entry.chSubscribe)) {
				...
				if (entry.aTime >= 0) f = function(uuid) {
					...
					try {
						...
						// Check if wait for finish and another instance is running
						...
						var res = __nam_execWithKillAfter(etry, chpsi, () => etry.exec(parent))
						...
						parent.publishRuntimeMetrics("plug", { trigger: "time", plugName: etry.getName(), plugType: etry.type, uuid: uuid })
					}
							...
							try {
								...
								if (cont) {
									...
									var execRes = (aobj, amap) => {
										return __nam_execWithKillAfter(etry, chpsi, () => etry.exec(aobj, amap))
									}

									// Handle set all operation if configured
								...
								if (cont) parent.publishRuntimeMetrics("plug", { trigger: "subscribe", plugName: etry.getName(), plugType: etry.type, uuid: aUUID, operation: aOp })
							}
						}
					}
					...
					if (isDef(entry.getCron())) {
						var f = function(uuid) {
							try {
								...
								$ch(parent.chPS).unset({ name: etry.getName(), uuid: uuid, start: chpsi })
								parent.publishRuntimeMetrics("plug", { trigger: "cron", plugName: etry.getName(), plugType: etry.type, uuid: uuid })
							}
