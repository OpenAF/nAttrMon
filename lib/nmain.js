...
var __NAM_RUNTIME_METRICS_PERSIST      = true       // Persist a bounded runtime metrics history snapshot to disk
var __NAM_RUNTIME_METRICS_HISTORY_SIZE = 60         // How many periodic runtime metrics points to retain/persist
...
__NAM_RUNTIME_METRICS_PERSIST = __nam_cfgBool(pms, "RUNTIME_METRICS_PERSIST", __NAM_RUNTIME_METRICS_PERSIST)
__NAM_RUNTIME_METRICS_HISTORY_SIZE = __nam_cfgNum(pms, "RUNTIME_METRICS_HISTORY_SIZE", __NAM_RUNTIME_METRICS_HISTORY_SIZE)
...
if (isMap(pms.RUNTIME_METRICS)) {
	...
	if (isBoolean(pms.RUNTIME_METRICS.persist)) __NAM_RUNTIME_METRICS_PERSIST = pms.RUNTIME_METRICS.persist
	if (isNumber(pms.RUNTIME_METRICS.historySize)) __NAM_RUNTIME_METRICS_HISTORY_SIZE = Number(pms.RUNTIME_METRICS.historySize)
}
...
const __nam_err = function(exception, rethrow, returnStr, code) {
	// Non-Error throws (e.g. raw Java exceptions) have no name/message; fall back to their string form.
	if (isUnDef(exception) || (isUnDef(exception.name) && isUnDef(exception.message))) {
		exception = { name: "Error", message: String(exception) }
	}
...
var nAttrMon = function(aConfigPath, debugFlag) {
	...
	this.chRuntimeHistory = "nattrmon::runtime::history"
	this.runtimeMetricsPersistEnabled = toBoolean(__NAM_RUNTIME_METRICS_PERSIST)
	this.runtimeMetricsHistorySize = isNumber(__NAM_RUNTIME_METRICS_HISTORY_SIZE) ? __NAM_RUNTIME_METRICS_HISTORY_SIZE : 60
	...
	if (this.runtimeMetricsEnabled) {
		...
		try { $ch(this.chRuntimeHistory).create(1, "simple") } catch(e) {}
	}
	...
	if (toBoolean(__NAM_NEED_CH_PERSISTENCE)) {
		...
		if (this.runtimeMetricsEnabled && this.runtimeMetricsPersistEnabled) {
			try {
				$ch(this.chRuntimeHistory).storeAdd(this.getSnapshotPath() + "/nattrmon.runtimehistory.snapshot", [ "t" ], true)
			} catch(e) {}
		}
	}
}
...
nAttrMon.prototype.publishRuntimeMetrics = function(aSource, aExtra) {
	...
	// Only append to the persisted history on the periodic tick -- watchdog/event-driven
	// publishes can burst during incidents and would otherwise write-amplify the snapshot file.
	if (this.runtimeMetricsPersistEnabled && _evt.source == "periodic") {
		try {
			var _point = {
				t: _evt.ts,
				channels: {
					currentValues: _snap.channels.currentValues,
					lastValues: _snap.channels.lastValues,
					warnings: _snap.channels.warnings,
					attributes: _snap.channels.attributes
				},
				scheduler: {
					threadsTotal: _snap.scheduler.threadsTotal,
					timedThreads: _snap.scheduler.timedThreads,
					cronThreads: _snap.scheduler.cronThreads,
					subscribedThreads: _snap.scheduler.subscribedThreads,
					staleThreads: _snap.scheduler.staleThreads
				},
				plugs: {
					total: _snap.plugs.total,
					running: _snap.plugs.running,
					totalExecutions: _snap.plugs.totalExecutions,
					totalErrors: _snap.plugs.totalErrors,
					avgExecTimeInMs: _snap.plugs.avgExecTimeInMs
				},
				degradedTotal: _diag.total,
				degradedNames: _diag.degraded.map(d => d.key),
				restarts: isNumber(_snap.restart.count) ? _snap.restart.count : 0
			}
			$ch(this.chRuntimeHistory).set({ t: _point.t }, _point)
			var _cap = isNumber(this.runtimeMetricsHistorySize) ? this.runtimeMetricsHistorySize : 60
			var _sz = $ch(this.chRuntimeHistory).size()
			if (_sz > _cap) {
				var _old = $from($ch(this.chRuntimeHistory).getAll()).sort("t").limit(_sz - _cap).select(r => ({ t: r.t }))
				if (_old.length > 0) $ch(this.chRuntimeHistory).unsetAll([ "t" ], _old)
			}
		} catch(e) {}
	}
}

// Returns the persisted/live bounded runtime metrics history (points ordered oldest -> newest)
// ----------------------------------------
nAttrMon.prototype.getRuntimeMetricsHistory = function() {
	try {
		return $from($ch(this.chRuntimeHistory).getAll()).sort("t").select()
	} catch(e) {
		return []
	}
}
...
// Validates one declarative object metadata entry. Metadata is intentionally
// separate from object constructors so it can be read without loading plug code.
// ----------------------------------------
// aMeta = metadata map
// Returns array of issue messages
// ----------------------------------------
nAttrMon.prototype.validateObjectMeta = function(aMeta) {
	var issues = []
	if (!isMap(aMeta)) return [ "Object metadata: expected a map." ]

	if ([ "input", "output", "validation" ].indexOf(aMeta.kind) < 0)
		issues.push("Object metadata: 'kind' must be input, output or validation.")
	if (!isString(aMeta.constructor) || String(aMeta.constructor).trim() == "")
		issues.push("Object metadata: 'constructor' must be a non-empty string.")
	if (!isString(aMeta.title) || String(aMeta.title).trim() == "")
		issues.push("Object metadata: 'title' must be a non-empty string for '" + _$(aMeta.constructor).default("unknown") + "'.")
	if (!isString(aMeta.description) || String(aMeta.description).trim() == "")
		issues.push("Object metadata: 'description' must be a non-empty string for '" + _$(aMeta.constructor).default("unknown") + "'.")
	if (isDef(aMeta.examples) && (!isArray(aMeta.examples) || aMeta.examples.some(p => !isString(p))))
		issues.push("Object metadata: 'examples' must be an array of strings for '" + _$(aMeta.constructor).default("unknown") + "'.")
	if (!isArray(aMeta.arguments)) {
		issues.push("Object metadata: 'arguments' must be an array for '" + _$(aMeta.constructor).default("unknown") + "'.")
	} else {
		aMeta.arguments.forEach((arg, idx) => {
			if (!isMap(arg)) {
				issues.push("Object metadata: argument #" + idx + " must be a map for '" + _$(aMeta.constructor).default("unknown") + "'.")
				return
			}
			if (!isString(arg.name) || String(arg.name).trim() == "") issues.push("Object metadata: argument #" + idx + " needs a non-empty 'name'.")
			if (!isString(arg.type) || String(arg.type).trim() == "") issues.push("Object metadata: argument '" + _$(arg.name).default(idx) + "' needs a non-empty 'type'.")
			if (!isString(arg.description) || String(arg.description).trim() == "") issues.push("Object metadata: argument '" + _$(arg.name).default(idx) + "' needs a non-empty 'description'.")
			if (isUnDef(arg.example)) issues.push("Object metadata: argument '" + _$(arg.name).default(idx) + "' needs an 'example'.")
			if (isDef(arg.required) && !isBoolean(arg.required)) issues.push("Object metadata: argument '" + _$(arg.name).default(idx) + "' has non-boolean 'required'.")
			if (isDef(arg.secret) && !isBoolean(arg.secret)) issues.push("Object metadata: argument '" + _$(arg.name).default(idx) + "' has non-boolean 'secret'.")
		})
	}
	return issues
}

// Loads metadata supplied by the installed package and, afterwards, the active
// configuration directory. A local entry with the same constructor overrides the
// packaged entry, enabling custom object documentation without modifying source.
// ----------------------------------------
// Returns an array with the loaded metadata entries
// ----------------------------------------
nAttrMon.prototype.loadObjectsMeta = function() {
	this.objectsMeta = {}
	this.objectsMetaIssues = []
	var paths = []
	var opackConfig = (getOPackPath("nAttrMon") || ".") + "/config/objects.meta"
	if (io.fileExists(opackConfig)) paths.push(opackConfig)
	var localConfig = this.configPath + "/objects.meta"
	if (io.fileExists(localConfig) && paths.indexOf(localConfig) < 0) paths.push(localConfig)
	...
	paths.forEach(basePath => {
		...
		.select(f => {
			try {
				var meta = f.filename.match(/\.json$/) ? io.readFileJSON(f.filepath) : io.readFileYAML(f.filepath, true)
				var issues = parent.validateObjectMeta(meta)
				if (issues.length > 0) {
					issues.forEach(issue => parent.objectsMetaIssues.push(f.filepath + ": " + issue))
					return
				}
				meta.source = f.filepath
				parent.objectsMeta[meta.constructor] = meta
			} catch(e) {
				parent.objectsMetaIssues.push(f.filepath + ": couldn't read object metadata: " + __nam_err(e, false, true))
			}
		})
	})

	return this.getObjectsMeta()
}

// Returns object metadata without exposing the mutable registry. Optionally
// filter by kind (input/output/validation) and constructor name.
nAttrMon.prototype.getObjectsMeta = function(aKind, aConstructor) {
	var entries = Object.keys(_$(this.objectsMeta).isMap().default({})).sort().map(k => clone(this.objectsMeta[k]))
	if (isDef(aKind)) entries = entries.filter(meta => meta.kind == aKind)
	if (isDef(aConstructor)) entries = entries.filter(meta => meta.constructor == aConstructor)
	return entries
}

// Returns metadata load/validation issues without exposing the internal array.
nAttrMon.prototype.getObjectsMetaIssues = function() {
	return clone(_$(this.objectsMetaIssues).isArray().default([]))
}
...
// Registers a named, independently-selectable history provider.
// ----------------------------------------
// aName     = provider name (used as the "source" selector)
// aProvider = object implementing getValuesByTime(name, secs) / getValuesByEvents(name, count)
// Returns the registered provider
// ----------------------------------------
nAttrMon.prototype.addHistoryProvider = function(aName, aProvider) {
	if (isUnDef(this.historyProviders)) this.historyProviders = {}
	this.historyProviders[aName] = aProvider
	return aProvider
}

// Gets a named history provider
// ----------------------------------------
// aName = provider name
// Returns the provider or undefined
// ----------------------------------------
nAttrMon.prototype.getHistoryProvider = function(aName) {
	return isDef(this.historyProviders) ? this.historyProviders[aName] : void 0
}

// Lists the names of all registered history providers
// ----------------------------------------
// Returns an array of provider names
// ----------------------------------------
nAttrMon.prototype.getHistoryProviderNames = function() {
	return isDef(this.historyProviders) ? Object.keys(this.historyProviders) : []
}

// Resolves which history provider should answer a query
// ----------------------------------------
// aSource = optional explicit provider name from the registry
// Returns the resolved provider or undefined
// ----------------------------------------
nAttrMon.prototype.resolveHistoryProvider = function(aSource) {
	var providers = isDef(this.historyProviders) ? this.historyProviders : {}

	if (isDef(aSource)) {
		if (isDef(providers[aSource])) return providers[aSource]
		this.debug("Unknown history source '" + aSource + "'.")
		return void 0
	}

	// Legacy fallback: a provider that only ever called setSessionData (never
	// addHistoryProvider) is still reachable and still wins the zero-config
	// default, so any config that predates this change keeps behaving exactly
	// as it does today.
	var legacy = this.getSessionData("attribute.history")

	var names = Object.keys(providers)
	if (names.length == 0) return legacy

	if (isDef(legacy)) {
		if (names.some(n => providers[n] !== legacy)) {
			this.debug("Multiple history providers registered (" + names.join(", ") + "); no 'source' specified, defaulting to the legacy attribute.history provider.")
		}
		return legacy
	}

	if (names.length > 1) {
		this.debug("Multiple history providers registered (" + names.join(", ") + ") and no 'source' specified; defaulting to '" + names[0] + "'. Pass an explicit 'source' to disambiguate.")
	}
...
nAttrMon.prototype.stop = function() {
	...
	if (isDef(this.sch)) this.sch.stop()
...
// aSource            = optional history provider name (see addHistoryProvider)
// Returns historical attribute values
// ---------------------------------------------------------------
nAttrMon.prototype.getHistoryValuesByTime = function(anAttributeName, howManySecondsAgo, aSource) {
	var attrHist = this.resolveHistoryProvider(aSource)
	if (isUnDef(attrHist)) {
...
// Returns historical attribute values
// ---------------------------------------------------------------
nAttrMon.prototype.getHistoryValuesByEvents = function(anAttributeName, howManyEventsAgo, aSource) {
	var attrHist = this.resolveHistoryProvider(aSource)
	if (isUnDef(attrHist)) {
		this.debug("An attribute.history is not defined.")
		return {}
